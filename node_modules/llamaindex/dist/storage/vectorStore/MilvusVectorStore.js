/* eslint-disable turbo/no-undeclared-env-vars */ import { MetadataMode } from "@llamaindex/core/schema";
import { DataType, MilvusClient } from "@zilliz/milvus2-sdk-node";
import { VectorStoreBase } from "./types.js";
import { metadataDictToNode, nodeToMetadata, parseArrayValue, parseNumberValue, parsePrimitiveValue } from "./utils.js";
function parseScalarFilters(scalarFilters) {
    ...
    for (const filter of scalarFilters.filters){
        switch(filter.operator){
            ...
            case ">=":
                {
                    filters.push(`metadata["${filter.key}"] ${filter.operator} ${parseNumberValue(filter.value)}`);
                    break;
                }
            default:
...
export class MilvusVectorStore extends VectorStoreBase {
    storesText = true;
    isEmbeddingQuery;
    flatMetadata = true;
    milvusClient;
