...
import { FilterOperator, VectorStoreBase, VectorStoreQueryMode } from "./types.js";
import { nodeToMetadata, parseArrayValue, parseNumberValue, parsePrimitiveValue } from "./utils.js";
...
const OPERATOR_TO_FILTER = {
    ...
    [FilterOperator.ANY]: ({ key, value }, metadata)=>{
        return parseArrayValue(value).some((v)=>parseArrayValue(metadata[key]).includes(v));
    },
    [FilterOperator.ALL]: ({ key, value }, metadata)=>{
        return parseArrayValue(value).every((v)=>parseArrayValue(metadata[key]).includes(v));
    },
    [FilterOperator.TEXT_MATCH]: ({ key, value }, metadata)=>{
        return parsePrimitiveValue(metadata[key]).includes(parsePrimitiveValue(value));
    },
...
export class SimpleVectorStore extends VectorStoreBase {
    storesText = false;
    data;
    persistPath;
    constructor(init){
