...
_export(exports, {
    AgentRunner: function() {
        return AgentRunner;
    },
    AgentWorker: function() {
        return AgentWorker;
    },
    MAX_TOOL_CALLS: function() {
        return MAX_TOOL_CALLS;
    },
    createTaskOutputStream: function() {
        return createTaskOutputStream;
    }
});
const _utils = require("@llamaindex/core/utils");
const _env = require("@llamaindex/env");
const _ChatHistory = require("../ChatHistory.js");
const _EngineResponse = require("../EngineResponse.js");
const _Settings = require("../Settings.js");
const _logger = require("../internal/logger.js");
...
function createTaskOutputStream(handler, context) {
    ...
    return new _env.ReadableStream({
        pull: async (controller)=>{
            ...
            _Settings.Settings.callbackManager.dispatchEvent("agent-start", {
                startStep: step
            });
            ...
            if (isLast) {
                ...
                _Settings.Settings.callbackManager.dispatchEvent("agent-end", {
                    endStep: step
                });
                controller.close();
            }
...
class AgentRunner {
    ...
    static defaultTaskHandler = async (step, enqueueOutput)=>{
        ...
        if (!stream) {
            ...
            await (0, _utils2.stepToolsStreaming)({
                response,
                tools,
                step,
                enqueueOutput
    ...
    get verbose() {
        return _Settings.Settings.debug || this.#verbose;
    }
    reset() {
        this.#chatHistory = [];
    }
    ...
    createTask(message, stream = false, verbose = undefined, chatHistory) {
        ...
        return this.#runner.createTask(message, {
            ...
            verbose === false ? _logger.emptyLogger : verbose || this.verbose ? _logger.consoleLogger : _logger.emptyLogger
        });
    }
    async chat(params) {
        let chatHistory = [];
        if (params.chatHistory instanceof _ChatHistory.ChatHistory) {
            chatHistory = params.chatHistory.messages;
        } else {
        ...
        for await (const stepOutput of task){
            ...
            if (stepOutput.isLast) {
                ...
                if ((0, _utils1.isAsyncIterable)(output)) {
                    return output.pipeThrough(new _env.TransformStream({
                        transform (chunk, controller) {
                            controller.enqueue(_EngineResponse.EngineResponse.fromChatResponseChunk(chunk));
                        }
                    }));
                } else {
                    return _EngineResponse.EngineResponse.fromChatResponse(output);
                }
            }
        }
