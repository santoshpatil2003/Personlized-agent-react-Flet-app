...
class LlamaParseReader extends _type.FileReader {
    ...
    constructor(params = {}){
        ...
        if (params.gpt4oMode) {
            params.gpt4oApiKey = params.gpt4oApiKey ?? (0, _env.getEnv)("LLAMA_CLOUD_GPT4O_API_KEY");
            this.gpt4oApiKey = params.gpt4oApiKey;
        }
        if (params.useVendorMultimodalModel) {
            params.vendorMultimodalApiKey = params.vendorMultimodalApiKey ?? (0, _env.getEnv)("LLAMA_CLOUD_VENDOR_MULTIMODAL_API_KEY");
   ...
   */ async loadJson(file) {
        let jobId;
        try {
            const data = await _env.fs.readFile(file);
            // Creates a job for the file
   ...
   */ async getImages(jsonResult, downloadPath) {
        try {
            ...
            try {
                await _env.fs.access(downloadPath);
            } catch  {
                await _env.fs.mkdir(downloadPath, {
                    recursive: true
                });
            }
            const images = [];
    ...
    async fetchAndSaveImage(imageName, imagePath, jobId) {
        ...
        await _env.fs.writeFile(imagePath, buffer);
    }
    static async getMimeType(data) {
        const typeinfos = (0, _magicbytes.filetypeinfo)(data);
