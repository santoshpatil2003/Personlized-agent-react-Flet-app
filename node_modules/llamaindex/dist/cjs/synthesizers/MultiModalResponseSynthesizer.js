...
class MultiModalResponseSynthesizer extends _Mixin.PromptMixin {
    ...
    async synthesize({ query, nodesWithScore, stream }) {
        ...
        if (stream) {
            ...
            return (0, _utils.streamConverter)(response, ({ text })=>_EngineResponse.EngineResponse.fromResponse(text, true, nodesWithScore));
        }
        const response = await llm.complete({
            prompt
        });
        return _EngineResponse.EngineResponse.fromResponse(response.text, false, nodesWithScore);
    }
}
