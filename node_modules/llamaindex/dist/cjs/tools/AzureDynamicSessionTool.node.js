...
Object.defineProperty(exports, "AzureDynamicSessionTool", {
    enumerable: true,
    get: function() {
        return AzureDynamicSessionTool;
    }
});
 ...
 */ async function getuserAgentSuffix() {
    try {
        //@ts-ignore
        const __filename1 = (0, _env.fileURLToPath)(require("url").pathToFileURL(__filename).toString());
        const __dirname = _env.path.dirname(__filename1);
        if (!_userAgent) {
            const data = await _env.fs.readFile(_env.path.join(__dirname, "..", "package.json"), "utf8");
...
function getAzureADTokenProvider() {
    return (0, _identity.getBearerTokenProvider)(new _identity.DefaultAzureCredential(), "https://dynamicsessions.io/.default");
}
...
class AzureDynamicSessionTool {
    outputDir = _env.path.normalize("tool-output");
    /**
   ...
   */ azureADTokenProvider;
    constructor(params){
        ...
        this.sessionId = params?.sessionId || (0, _env.randomUUID)();
        this.poolManagementEndpoint = params?.poolManagementEndpoint || ((0, _env.getEnv)("AZURE_CONTAINER_APP_SESSION_POOL_MANAGEMENT_ENDPOINT") ?? "");
        this.azureADTokenProvider = params?.azureADTokenProvider ?? getAzureADTokenProvider();
        if (!this.poolManagementEndpoint) {
            throw new Error("AZURE_CONTAINER_APP_SESSION_POOL_MANAGEMENT_ENDPOINT must be defined.");
        }
    }
    _buildUrl(path) {
        ...
        try {
            ...
            if (response.body) {
                ...
                if (params.localFilename) {
                    const writer = (0, _env.createWriteStream)(_env.path.resolve(params.localFilename));
                    const blob = await response.blob();
                    _env.Readable.from(blob.stream()).pipe(writer);
                    return;
                }
   ...
   */ async saveToDisk(base64Data, ext) {
        try {
            const filename = `${(0, _env.randomUUID)()}.${ext}`;
            const buffer = Buffer.from(base64Data, "base64");
            const outputPath = await this.getOutputPath(filename);
            await _env.fs.writeFile(outputPath, buffer);
   ...
   */ async getOutputPath(filename) {
        if (await this.exists(this.outputDir) === false) {
            try {
                await _env.fs.mkdir(this.outputDir, {
                    recursive: true
                });
        ...
        return _env.path.join(this.outputDir, filename);
    }
    /**
