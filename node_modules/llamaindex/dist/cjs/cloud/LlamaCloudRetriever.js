...
class LlamaCloudRetriever {
    ...
    clientParams;
    retrieveParams;
    projectName = _constants.DEFAULT_PROJECT_NAME;
    pipelineName;
    ...
    async retrieve({ query, preFilters }) {
        ...
        const pipeline = await _api.PipelinesService.getPipelineApiV1PipelinesPipelineIdGet({
            pipelineId: pipelines[0].id
        });
        if (!pipeline) {
            throw new Error(`No pipeline found with name ${this.pipelineName} in project ${this.projectName}`);
        }
        const results = await _api.PipelinesService.runSearchApiV1PipelinesPipelineIdRetrievePost({
        ...
        const nodesWithScores = this.resultNodesToNodeWithScore(results.retrieval_nodes);
