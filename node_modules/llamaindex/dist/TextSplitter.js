import { EOL, tokenizers } from "@llamaindex/env";
// GitHub translated
...
export const defaultSentenceTokenizer = (text)=>{
    const slist = [];
 ...
 * @param text
 * @returns string[]
 */ export function cjkSentenceTokenizer(sentence) {
    const slist = [];
 ...
 */ export class SentenceSplitter {
    ...
    splitLongSentences;
    constructor(options){
        const { chunkSize = DEFAULT_CHUNK_SIZE, chunkOverlap = DEFAULT_CHUNK_OVERLAP, tokenizer = null, paragraphSeparator = defaultParagraphSeparator, chunkingTokenizerFn, splitLongSentences = false } = options ?? {};
    ...
    getEffectiveChunkSize(extraInfoStr) {
        ...
        if (extraInfoStr != undefined) {
            const numExtraTokens = this.tokenizer.encode(`${extraInfoStr}\n\n`).length + 1;
        ...
        for (const split of sentenceSplits){
            ...
            if (splitLen <= effectiveChunkSize) {
                ...
                for(let i = 0; i < splitLen; i += effectiveChunkSize){
                    const cur_split = this.tokenizer.decode(splitTokens.slice(i, i + effectiveChunkSize));
                    newSplits.push({
                        text: cur_split,
