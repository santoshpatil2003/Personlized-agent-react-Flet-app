import { Document, ImageDocument, NodeRelationship, TextNode } from "@llamaindex/core/schema";
 ...
 */ export function getNodesFromDocument(doc, textSplitter, includeMetadata = true, includePrevNextRel = true) {
    ...
    textSplits.forEach((textSplit)=>{
        const node = new TextNode({
            ...
            excludedEmbedMetadataKeys: _.cloneDeep(document.excludedEmbedMetadataKeys),
    ...
    if (includePrevNextRel) {
        nodes.forEach((node, index)=>{
            if (index > 0) {
                node.relationships[NodeRelationship.PREVIOUS] = nodes[index - 1].asRelatedNodeInfo();
            }
            if (index < nodes.length - 1) {
                node.relationships[NodeRelationship.NEXT] = nodes[index + 1].asRelatedNodeInfo();
            }
        });
    }
    return nodes;
