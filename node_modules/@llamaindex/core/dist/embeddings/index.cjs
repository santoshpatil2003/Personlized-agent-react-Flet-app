...
var index_cjs$1 = require('../schema/index.cjs');
...
function truncateMaxTokens(tokenizer, value, maxTokens) {
    ...
    const t = env.tokenizers.tokenizer(tokenizer);
...
class BaseEmbedding {
    similarity(embedding1, embedding2, mode = exports.SimilarityType.DEFAULT) {
        return similarity(embedding1, embedding2, mode);
    }
    async getQueryEmbedding(query) {
        const text = index_cjs.extractSingleText(query);
        if (text) {
   ...
   */ async getTextEmbeddingsBatch(texts, options) {
    ...
    async transform(nodes, options) {
        const texts = nodes.map((node)=>node.getContent(index_cjs$1.MetadataMode.EMBED));
